...
macOS/Linux: python3 -m pip install python-dotenv
```

It is already listed in requirements.txt; ensure your active interpreter matches the one running the app.
VS Code: select the correct Python interpreter (bottom-right) or set "envFile" in `.vscode/launch.json`.
Docker: verify docker-compose uses `env_file: .env` for the backend service.
